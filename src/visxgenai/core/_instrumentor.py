"""
Extremely lazy, but working patch for making all Langfuse traces generated via DSPY public by setting {'langfuse.trace.public': True} OpenTelemetry attribute manually.

Adapted code: https://github.com/Arize-ai/openinference/blob/eb4d59db5c4d51cc29eb163f5607a91305a5b430/python/instrumentation/openinference-instrumentation-dspy/src/openinference/instrumentation/dspy/__init__.py
"""
...
class _ModuleForwardWrapper(_WithTracer):
    ...
    def __call__(
        ...
        with self._tracer.start_as_current_span(
            ...
            span.set_attributes(
                dict(
                    _flatten(_module_prediction_output_attributes(prediction, instance))
                )
                | {"langfuse.trace.public": True}
            )
            span.set_status(StatusCode.OK)
                ...
                _flatten(
                    {
                        ...
                        **(
                            ...
                            if (
                                forward_method := getattr(
                                    instance.__class__, "aforward", None
                                )
                            )
                            else {}
                        ),
...
def _llm_invocation_parameters(
    ...
    combined_kwargs.pop("vertex_credentials", None)
    combined_kwargs.pop("vertex_location", None)
...
def _module_prediction_output_attributes(
    ...
    if isinstance(prediction, dspy.Prediction):
        ...
        output_attributes[OUTPUT_VALUE] = safe_json_dumps(
