import { L as z, E as c, a as Y } from "./index-Cj6KsLYN.js";
import { s as g, t as Q } from "./highlight-Dq4mfqgD.js";
...
function $(O) {
	return i(O) || (O >= 97 && O <= 102) || (O >= 65 && O <= 70);
}
const n = (O, e, p) => (a, y) => {
		for (let t = !1, o = 0, d = 0; ; d++) {
			let { next: r } = a;
			if (l(r) || r == S || r == m || (t && i(r)))
				!t && (r != S || d > 0) && (t = !0),
					o === d && r == S && o++,
					a.advance();
			else if (r == _ && a.peek(1) != V) {
				if ((a.advance(), $(a.next))) {
					do a.advance();
					while ($(a.next));
					a.next == 32 && a.advance();
				...
				t && a.acceptToken(o == 2 && y.canShift(P) ? e : r == q ? p : O);
				break;
			}
		}
	},
	...
	w = new c((O) => {
		if (!X.includes(O.peek(-1))) {
			let { next: e } = O;
			if ((e == x && (O.advance(), O.acceptToken(s)), l(e))) {
				do O.advance();
				while (l(O.next) || i(O.next));
				O.acceptToken(s);
			}
		}
	}),
	...
	F = { __proto__: null, or: 98, and: 98, not: 106, only: 106, layer: 170 },
	D = { __proto__: null, selector: 112, layer: 166 },
	B = {
		__proto__: null,
		"@import": 162,
		"@media": 174,
	...
	H = z.deserialize({
		...
		tokenizers: [
			...
			3,
			4,
			new Y("m~RRYZ[z{a~~g~aO#v~~dP!P!Qg~lO#w~~", 28, 129),
		],
		topRules: { StyleSheet: [0, 6], Styles: [1, 105] },
		specialized: [
			{ term: 124, get: (O) => I[O] || -1 },
			{ term: 125, get: (O) => F[O] || -1 },
			{ term: 4, get: (O) => D[O] || -1 },
			{ term: 25, get: (O) => B[O] || -1 },
			{ term: 123, get: (O) => K[O] || -1 },
		],
		tokenPrec: 1963,
	});
export { H as parser };
