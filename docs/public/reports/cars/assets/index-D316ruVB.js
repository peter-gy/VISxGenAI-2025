import { L as s, E as r, a as e, C as o } from "./index-C3V7iUvF.js";
import { s as X, t as O } from "./highlight-BkuM5Bn6.js";
...
const c = 315,
	...
	G = new r(
		(Q, i) => {
			let { next: a } = Q;
			(a == p || a == -1 || i.context) && Q.acceptToken(q);
		},
	...
	w = new r(
		(Q, i) => {
			...
			P.indexOf(a) > -1 ||
				(a == S && (($ = Q.peek(1)) == S || $ == h)) ||
				(a != p && a != y && a != -1 && !i.context && Q.acceptToken(c));
		},
		{ contextual: !0 },
	),
	z = new r(
		(Q, i) => {
			Q.next == T && !i.context && Q.acceptToken(t);
		},
		{ contextual: !0 },
	),
	U = new r(
		(Q, i) => {
			...
			if (a == v || a == R) {
				...
				a == j &&
					...
					(Q.advance(),
					Q.advance(),
					(Q.next < 48 || Q.next > 57) && Q.acceptToken(x));
		},
		{ contextual: !0 },
	);
function Z(Q, i) {
...
const W = new r((Q, i) => {
		if (Q.next != b || !i.dialectEnabled(f) || (Q.advance(), Q.next == S))
			return;
		let a = 0;
		for (; P.indexOf(Q.next) > -1; ) Q.advance(), a++;
		if (Z(Q.next, !0)) {
			for (Q.advance(), a++; Z(Q.next, !1); ) Q.advance(), a++;
			for (; P.indexOf(Q.next) > -1; ) Q.advance(), a++;
			if (Q.next == d) return;
			for (let $ = 0; ; $++) {
				if ($ == 7) {
					if (!Z(Q.next, !0)) return;
					break;
				}
				if (Q.next != "extends".charCodeAt($)) break;
				Q.advance(), a++;
			}
		}
		Q.acceptToken(Y, -a);
	...
	M = X({
		...
		"NewExpression/VariableName": O.className,
		...
		"is keyof unique infer asserts": O.operatorKeyword,
	...
	K = s.deserialize({
		...
		tokenizers: [
			...
			new e("j~RQYZXz{^~^O(P~~aP!P!Qd~iO(Q~~", 25, 322),
		],
		topRules: {
			Script: [0, 7],
		...
		specialized: [
			{ term: 326, get: (Q) => L[Q] || -1 },
			{ term: 342, get: (Q) => E[Q] || -1 },
			{ term: 95, get: (Q) => A[Q] || -1 },
		],
		tokenPrec: 15124,
	});
