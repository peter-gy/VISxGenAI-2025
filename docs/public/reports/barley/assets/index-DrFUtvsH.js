...
function* yt(e) {
	...
	for (const t in e)
		!Le(t) && ge(e, t) && (yield R(t, v(e, t), "observablehq--key"));
...
function* Rt(e) {
	...
	n > t + 1 && (yield le(n - t - 1));
	for (const i in e)
		!Le(i) && ge(e, i) && (yield D(i, v(e, i), "observablehq--key"));
...
const _i = Object.freeze(
	Object.defineProperty(
		{ __proto__: null, canvas: pi, context2d: fi, svg: bi, text: hi, uid: Zi },
		Symbol.toStringTag,
		{ value: "Module" },
	),
);
...
const wi = () =>
	...
	Fi = () =>
		C(() => import("./duckdb-DUtxcltl.js"), [], import.meta.url).then(
			(e) => e.DuckDBClient,
		),
	Ni = () =>
		C(
	...
	Si = () => C(() => import("./leaflet-D54mqpIu.js"), [], import.meta.url),
	xi = () =>
		C(() => import("./mapboxgl-C94UFoDx.js"), [], import.meta.url).then(
			(e) => e.default,
		),
	Ji = () =>
		C(() => import("./md-el_xK5xW.js"), [], import.meta.url).then((e) => e.md),
	Ti = () =>
...
function L(e, t, n = bn) {
		...
		if (
			(G.define(
				...
				() => {
					const y = g._version;
					return (E) => {
						if (y < p) throw new Error("stale display");
						if (e.variables[0] !== g) throw new Error("stale display");
						return y > p && O(e), (p = y), Oe(e, E), E;
					};
				},
			),
	...
	if ((u.push(g.define(h, r, o)), l != null)) {
		if (s) {
			const p = Je(l, "viewof$");
			u.push(K.define(p, [l], Ae));
		} else if (d) {
			const p = Je(l, "mutable "),
				G = `cell ${i}`;
			g.define(p, [G], ([y]) => y),
				u.push(
					K.define(l, r, o),
					K.define(G, [l], gi),
					K.define(`mutable$${p}`, [G], ([, y]) => y),
				);
		}
	} else for (const p of a) u.push(K.variable(!0).define(p, [h], (G) => G[p]));
}
function Je(e, t) {
	...
	{ root: document.getElementById("cell-8"), expanded: [], variables: [] },
	{
		...
		body: async (e, t, n, i, o, r, a, l) => {
			const c = {
					...
					summary:
						"This view calculates the average crop yield for each variety over the years, enabling the analysis of temporal performance trends and comparisons between different crop types.",
					fields: [
						{
							name: "year_date",
							label: "Year",
							description:
								"Derived from year: The calendar year of the harvest, converted to a DATE format (YYYY-01-01) for temporal analysis.",
						...
						},
						{
							...
							label: "Average Yield",
							description:
								"Derived from yield: The average crop yield, calculated by averaging the 'yield' for each variety within each year. NULL values in 'yield' are excluded from the calculation.",
				...
				s = {
					...
					facet: {
						row: { field: "year_date", title: "Year", type: "temporal" },
					},
					spec: {
						mark: { type: "tick" },
						encoding: {
			...
			const u = "dataset_01",
				g =
					"https://visxgenai-cdn.peter.gy/sessions/terrifying-pueblo-confessed-solidly/artifacts/datasets.parquet";
			await i(o, u, g);
			const h = await r({ table: u, spec: s, metadata: c, coordinator: a });
			l(h.viewClient.node()), l(h.tableClient.node());
			const p = document.getElementById("loading-skeleton-dataset_01");
			return (
				p && p.remove(),
				{
	...
	{ root: document.getElementById("cell-12"), expanded: [], variables: [] },
	{
		...
		body: async (e, t, n, i, o, r, a, l) => {
			const c = {
					...
					summary:
						"A ranked list of agricultural sites based on their average crop yield, from highest to lowest, to identify the most productive locations.",
					fields: [
						{
							name: "site",
							label: "Site",
							description:
								"Derived from [site]: The agricultural site location. Leading and trailing whitespace has been removed, and any NULL or empty values are replaced with 'Unknown'.",
							semantic_type: "Name",
							role: "primary",
						},
						{
							...
							label: "Average Yield",
							description:
								"Derived from [yield]: The average crop yield, aggregated by site. Any rows with a NULL yield were excluded from the calculation to ensure accuracy.",
							semantic_type: "Numeric",
							role: "primary",
						},
					],
				...
				s = {
					...
					encoding: {
						...
						y: { field: "site", sort: "-x", title: "Site", type: "ordinal" },
					},
					$schema: "https://vega.github.io/schema/vega-lite/v6.1.0.json",
				};
			e(
			...
			const h = await r({ table: u, spec: s, metadata: c, coordinator: a });
			l(h.viewClient.node()), l(h.tableClient.node());
			const p = document.getElementById("loading-skeleton-dataset_02");
			return (
				p && p.remove(),
				{
	...
	{ root: document.getElementById("cell-16"), expanded: [], variables: [] },
	{
		...
		body: async (e, t, n, i, o, r, a, l) => {
			const c = {
					...
					summary:
						"A detailed view of individual crop yields, segmented by agricultural site and crop variety, to analyze production distribution and variability.",
					fields: [
						{
							name: "yield",
							label: "Yield",
						...
						},
						{
							...
							label: "Harvest Date",
							description:
								"Derived from year: Converted to a DATE type using MAKE_DATE(year, 1, 1).",
							semantic_type: "DateTime",
							role: "detail",
						},
					],
				...
				s = {
					...
					spec: {
						...
						encoding: {
							...
							tooltip: [
								{ field: "yield", title: "Yield", type: "quantitative" },
								{ field: "site", title: "Site", type: "nominal" },
								{ field: "variety", title: "Variety", type: "nominal" },
								{
									field: "harvest_date",
			...
			const h = await r({ table: u, spec: s, metadata: c, coordinator: a });
			l(h.viewClient.node()), l(h.tableClient.node());
			const p = document.getElementById("loading-skeleton-dataset_03");
			return (
				p && p.remove(),
				{
	...
	{ root: document.getElementById("cell-19"), expanded: [], variables: [] },
	{
		...
		body: async (e, t, n, i) => {
			const [
				...
				p = await e.of(),
				G = new r.Coordinator();
			G.databaseConnector(r.wasmConnector({ duckdb: p._db }));
			function y(I, m, A) {
      ...
      `
				...
				const j = await o(m, {
					...
					P = await Be(f, { coordinator: Z, table: m, filterBy: j.filterBy });
