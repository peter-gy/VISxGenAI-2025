...
class v {
	...
	reduce(t) {
		...
		if ((n && (this.score += n), s == 0)) {
			this.pushState(h.getGoto(this.state, i, !0), this.reducePos),
				i < h.minRepeatTerm &&
					this.storeNode(i, this.reducePos, this.reducePos, r ? 8 : 4, !0),
	...
	storeNode(t, e, s, i = 4, h = !1) {
		if (
			...
			if (
				(n == 0 &&
					r.parent &&
					((n = r.bufferBase - r.parent.bufferBase), (r = r.parent)),
	...
	shift(t, e, s, i) {
		if (t & 131072) this.pushState(t & 65535, this.pos);
			...
			(i > this.pos || e <= r.maxNode) &&
				((this.pos = i), r.stateFlag(h, 1) || (this.reducePos = i)),
				this.pushState(h, s),
	...
	get deadEnd() {
		...
		let { parser: t } = this.p;
		return (
			t.data[t.stateSlot(this.state, 1)] == 65535 && !t.stateSlot(this.state, 4)
		);
	}
	restart() {
		this.storeNode(0, this.pos, this.pos, 4, !0),
...
class H {
	...
	peek(t) {
		...
		if (e >= 0 && e < this.chunk.length)
			...
			if (
				((s = h),
				s >= this.chunk2Pos && s < this.chunk2Pos + this.chunk2.length)
			)
				...
				(this.chunk2 = this.input.chunk((this.chunk2Pos = s))),
					s + this.chunk2.length > n.to &&
						(this.chunk2 = this.chunk2.slice(0, n.to - s)),
...
class W {
	constructor(t, e, s) {
		(this.precTable = e),
			(this.elseToken = s),
			(this.data = typeof t == "string" ? x(t) : t);
	}
	token(t, e) {
		let s = t.pos,
			i = 0;
		for (;;) {
			...
			if (
				(O(this.data, t, e, 0, this.data, this.precTable), t.token.value > -1)
			)
				break;
		...
		if (t) {
			for (
				this.safeFrom = t.openStart
					? D(t.tree, t.from + t.offset, 1) - t.offset
					: t.from,
					this.safeTo = t.openEnd
						? D(t.tree, t.to + t.offset, -1) - t.offset
						: t.to;
				this.trees.length;
			)
			...
			for (let a = r.stateSlot(h, o ? 2 : 1); ; a += 3) {
				if (n[a] == 65535)
					if (n[a + 1] == 1) a = k(n, a + 2);
						...
						i == 0 &&
							n[a + 1] == 2 &&
							(i = this.putAction(k(n, a + 2), e, s, i));
						break;
					}
...
class Q {
	constructor(t, e, s, i) {
		...
		(this.stacks = [v.start(this, t.top[0], h)]),
			(this.fragments =
				s.length && this.stream.end - h > t.bufferLength * 4
					? new J(s, t.nodeSet)
					: null);
	}
	get parsedPos() {
	...
	advance() {
		...
		if (!s.length) {
			...
			if (r)
				return (
					g && console.log("Finish with " + this.stackID(r)),
					this.stackToTree(r)
				);
		...
		if (this.recovering && i) {
			let r =
				this.stoppedAt != null && i[0].pos > this.stoppedAt
					? i[0]
					: this.runRecovery(i, h, s);
			if (r)
				return (
					g && console.log("Force-finish " + this.stackID(r)),
					this.stackToTree(r.forceAll())
				);
		}
		if (this.recovering) {
			let r = this.recovering == 1 ? 1 : this.recovering * 3;
			if (s.length > r)
				for (s.sort((n, o) => o.score - n.score); s.length > r; ) s.pop();
			...
			t: for (let r = 0; r < s.length - 1; r++) {
				...
				for (let o = r + 1; o < s.length; o++) {
					...
					if (
						n.sameState(a) ||
						(n.buffer.length > 500 && a.buffer.length > 500)
					)
			...
			for (let f = this.fragments.nodeAt(i); f; ) {
				...
				if (p > -1 && f.length && (!a || (f.prop(y.contextHash) || 0) == u))
					return (
						...
						g &&
							console.log(
								r + this.stackID(t) + ` (via reuse of ${h.getName(f.type.id)})`,
							),
						!0
					);
			...
			return (
				...
				g &&
					console.log(
						r +
							this.stackID(t) +
							` (via always-reduce ${h.getName(n & 65535)})`,
					),
				!0
			);
			...
			if (
				...
				g &&
					console.log(
						r +
							this.stackID(d) +
							` (via ${(u & 65536) == 0 ? "shift" : `reduce of ${h.getName(u & 65535)}`} for ${h.getName(f)} @ ${i}${d == t ? "" : ", split"})`,
					),
				c)
			)
				return !0;
		...
		for (let r = 0; r < t.length; r++) {
			...
			if (
				...
				(h ||
					((h = !0),
					n.restart(),
					g && console.log(u + this.stackID(n) + " (restarted)"),
			...
			for (
				...
				f.forceReduce() &&
				c < 10 &&
				(g && console.log(p + this.stackID(f) + " (via force-reduce)"),
				!this.advanceFully(f, s));
				c++
			)
				g && (p = this.stackID(f) + " -> ");
			for (let c of n.recoverByInsert(o))
				g && console.log(u + this.stackID(c) + " (via recover-insert)"),
...
class w extends M {
	constructor(t) {
		...
		(this.specializers = this.specializerSpecs.map(F)),
			...
			(this.tokenizers = t.tokenizers.map((n) =>
				typeof n == "number" ? new m(r, n) : n,
			)),
